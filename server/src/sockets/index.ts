...
export const handleSocketConnection = (socket: Socket, io: Server) => {
  ...
  // Join a lesson room
  socket.on('join-lesson', async (data: { sessionId: string, userId: string }) => {
    try {
  ...
  // Handle step change
  socket.on('change-step', async (data: { sessionId: string; newStep: number }) => {
    try {
  ...
  socket.on('change-speaker', async (data: { sessionId: string; speakerId: string | null }) => {
    try {
